Zepto(function($){
  //点击进入页面判断
    ...
    if(getData!==""){
        ...
        if(formdatas.sexId=='男'){
           $("#editForm .getSex span").text("男").attr("dataval",formdatas.sexId);
        }else {
           $("#editForm .getSex span").text("女").attr("dataval",formdatas.sexId);
        }
    ...
    $('.sexbox input[type="radio"]').on('click',function(){
        ...
        //获取表单值
    function getVal(box){
        var _token = box.find('input[name="_token"]').val();
        var username=box.find(".username").val();
        var sexId = box.find(".isActive").attr("dataval");
        var birthday = box.find(" .birthday").val();
        ...
        if(username!==""&&sexId!==""&&birthday!==""&&cardId!==""&&address!==""&&school!==""&&user!==""&&phoneNum!==""){
            var formdata={
                "_token":_token,
                "username":username,
                "sexId":sexId,
            ...
            $.post("/students", formdata, function(result){
